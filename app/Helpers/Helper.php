...
if (!function_exists('theme')) {
    function theme($fileName)
    {
        if (app()->bound('getSetting')) {
            ...
            return 'frontend' . '.trainingsector.' . $fileName;
        }
    }
}
...
if (!function_exists('isSubscribe')) {
    ...
    {
        if (moduleStatusCheck('Subscription') && Auth::check()) {
            $user = Auth::user();
...
if (!function_exists('hasTax')) {
    function hasTax()
    {
        if (moduleStatusCheck('Tax')) {
            $tax = new \Modules\Tax\Http\Controllers\TaxSettingController();
            return $tax->status();
        }
        return false;
    }
...
if (!function_exists('applyTax')) {
    ...
    {
        $tax = \Modules\Tax\Entities\TaxSetting::first();
        $vat = $tax->percentage ?? 0;
